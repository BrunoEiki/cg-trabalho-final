...
const DrawingGrid = () => {
	...
	const [scaleFixedPoint, setScaleFixedPoint] = useState(null);
	const [selectingScalePoint, setSelectingScalePoint] = useState(false);
	...
	const scaleGrid = () => {
		if (!scaleFixedPoint) {
			alert("Por favor, selecione um ponto fixo primeiro!");
			return;
		}

		const [fixedX, fixedY] = scaleFixedPoint;
		...
		const newGrid = Array(GRID_SIZE)
			.fill()
			.map(() => Array(GRID_SIZE).fill(null));
		const scaledPoints = new Set(); // To keep track of points we've already processed

		// First pass: Calculate new positions for all filled cells
		const pointsToScale = [];
		for (let i = 0; i < GRID_SIZE; i++) {
			for (let j = 0; j < GRID_SIZE; j++) {
				if (grid[i][j]) {
					// Calculate relative position to fixed point
					const relativeX = i - fixedX;
					const relativeY = j - fixedY;

					// Apply scaling
					const scaledX = Math.round(relativeX * sx);
					const scaledY = Math.round(relativeY * sy);

					// Calculate new absolute position
					const newI = scaledX + fixedX;
					const newJ = scaledY + fixedY;
		...
		// Second pass: Fill in all points between original and scaled positions
		pointsToScale.forEach(({ original, scaled }) => {
			const [origX, origY] = original;
			const [newX, newY] = scaled;

			// Get all points along the scaling path
			const interpolatedPoints = getPointsBetween(
				origX,
				origY,
				newX,
				newY
			...
			// If scaling up, fill all points
			if (sx > 1 || sy > 1) {
				interpolatedPoints.forEach(([x, y]) => {
					if (x >= 0 && x < GRID_SIZE && y >= 0 && y < GRID_SIZE) {
						newGrid[x][y] = grid[origX][origY];
						scaledPoints.add(`${x},${y}`);
					}
				});
			}
			// If scaling down, only fill the end points
			else {
				if (
					newX >= 0 &&
					newX < GRID_SIZE &&
					...
					newGrid[newX][newY] = grid[origX][origY];
					scaledPoints.add(`${newX},${newY}`);
				}
			}
		});
	...
	const getPointsBetween = (x1, y1, x2, y2) => {
		...
		// Bresenham's line algorithm to get all points between
		let x = x1;
		let y = y1;
	...
	const handleCellClick = (row, col) => {
		...
		if (scaleMode && selectingScalePoint) {
			setScaleFixedPoint([row, col]);
			setSelectingScalePoint(false);
			return;
		}
	...
	const handleScaleClick = () => {
		...
		if (!scaleMode) {
			...
			setBoolFillAlgorithm(false);
			if (!scaleFixedPoint) {
				setSelectingScalePoint(true);
			}
		} else {
			setSelectingScalePoint(false);
			setScaleFixedPoint(null);
			setScaleX(1);
			setScaleY(1);
		}
	};
			...
			<div className="flex flex-col gap-2">
				...
				{scaleMode && (
					<div className="flex flex-col gap-2 p-2 border rounded">
						{selectingScalePoint ? (
							<div className="text-sm text-blue-600">
								Clique na grade para selecionar o ponto fixo
							</div>
						) : (
							<>
								<div className="flex flex-col gap-1">
